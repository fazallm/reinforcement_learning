...
class PPO:
    def __init__(self, env, n_latent_var, lr, betas, gamma, K_epochs, eps_clip, checkpoint='tmp/ppo'):
        self.lr = lr
        self.betas = betas
        self.gamma = gamma
        ...
        if isinstance(env.action_space, Discrete):
            ...
            self.policy = ActorCriticContinuous(env.reset().reshape(-1).shape[0], env.action_space.shape[0], n_latent_var, checkpoint+'/policy').to(device)
            self.policy_old = ActorCriticContinuous(env.reset().reshape(-1).shape[0], env.action_space.shape[0], n_latent_var, checkpoint+'/old_policy').to(device)
            self.policy_old.load_state_dict(self.policy.state_dict())
